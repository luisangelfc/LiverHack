// ==========================================================
// HRBP
// Controlador/visualizador global del flujo de la vacante.
//
// Consume:
//   ../js/candidatos-db.js
//   ../js/edat-flow.js
...
// ==========================================================
// ENCABEZADO Y ESTADO DE LA VACANTE
// ==========================================================
...
function hrbpRenderVacante() {
    ...
    hrbpEl("vacancyState").innerHTML = `
        <svg class="talento-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><rect x="3" y="5" width="18" height="16" rx="2"/><path d="M7 3v4M17 3v4M3 10h18M7 14h3M14 14h3M7 17h3"/></svg>
        ${etiquetaVacante}
    `;
    ...
    const tarjetas = [
        {
            ...
            nota: estado.requisicion ? EDAT.fecha(estado.requisicion.fecha) : "La abre HRBP"
        },
        {
            titulo: "ALINEACIÓN AT",
            valor: estado.alineacion.at.estado,
            nota: estado.alineacion.at.fecha ? EDAT.fecha(estado.alineacion.at.fecha) : "En espera"
        },
        {
            titulo: "ALINEACIÓN HM",
            valor: estado.alineacion.hm.estado,
            nota: estado.alineacion.hm.fecha ? EDAT.fecha(estado.alineacion.hm.fecha) : "En espera"
        },
        {
            ...
            return `
                <article class="timeline-step ${clase}">
                    ...
                    <div class="timeline-content">
                        ...
                        <p>${detalle[etapa.clave]}</p>

                        <span class="resolution-time">${etapa.responsable}</span>
...
function hrbpDetallePorEtapa() {
    ...
    return {
        ...
        alineacion: `Atracción de Talento: ${estado.alineacion.at.estado} · Hiring Manager: ${estado.alineacion.hm.estado}.`,
...
function hrbpRenderRequisicion() {
    ...
    formulario.querySelectorAll("input, textarea, button").forEach(campo => {
        campo.disabled = enviada;
    });

    if (enviada) {

        hrbpEl("reqTitulo").value = estado.requisicion.titulo;
        hrbpEl("reqEscolaridad").value = estado.requisicion.escolaridad;
        hrbpEl("reqDescripcion").value = estado.requisicion.descripcion;
        hrbpEl("reqConocimientos").value = estado.requisicion.conocimientos;
        hrbpEl("reqHard").value = estado.requisicion.hardSkills;
        hrbpEl("reqSoft").value = estado.requisicion.softSkills;
        hrbpEl("reqSalario").value = estado.requisicion.salario;
        hrbpEl("reqDsq").value = estado.requisicion.dsq;
    ...
    hrbpEl("alignmentGrid").innerHTML = [
        ...
        },
        {
            ...
            valor: EDAT.vacanteDetenida() ? "Detenida" : estado.vacante.estado,
        ...
        .map(
            caja => `
                <article class="decision-box">
                    <span>${caja.titulo}</span>
                    <strong>${caja.valor}</strong>
                    <small>${caja.nota}</small>
                </article>
            `
        )
        .join("");

    const noNegociables = estado.alineacion.at.noNegociables;

    hrbpEl("requirementList").innerHTML = noNegociables.length === 0
        ? '<li>Atracción de Talento aún no define los no negociables.</li>'
        : noNegociables.map(item => `<li>${item}</li>`).join("");
...
function hrbpInitRequisicion() {
    ...
    hrbpEl("requisitionForm").addEventListener("submit", evento => {

        evento.preventDefault();

        const datos = {
            titulo: hrbpEl("reqTitulo").value.trim(),
            escolaridad: hrbpEl("reqEscolaridad").value.trim(),
            descripcion: hrbpEl("reqDescripcion").value.trim(),
            conocimientos: hrbpEl("reqConocimientos").value.trim(),
            hardSkills: hrbpEl("reqHard").value.trim(),
            softSkills: hrbpEl("reqSoft").value.trim(),
            salario: hrbpEl("reqSalario").value.trim(),
            dsq: hrbpEl("reqDsq").value.trim()
        };

        const faltante = Object.values(datos).some(valor => valor === "");

        if (faltante) {
            hrbpToast("Completa todos los campos de la requisición.");
            return;
        }

        const resultado = EDAT.enviarRequisicion(datos);

        hrbpToast(resultado.mensaje);

    });

    hrbpEl("goRequisitionButton").addEventListener("click", () => {
...
function hrbpRenderCandidatos() {
    ...
    const candidatos = EDAT.candidatos();
    const estado = EDAT.estado();

    const activos = EDAT.candidatosActivos().length;

    hrbpEl("candidatesCounter").textContent =
        `${candidatos.length} perfiles en la base · ${activos} entraron al flujo de esta vacante.`;

    hrbpEl("candidateTrack").innerHTML = candidatos
        .map(candidato => {
            ...
            if (estado.seleccionHM.includes(candidato.id)) {
                detalles.push("Enviado a entrevista con Hiring Manager");
            }

            if (candidato.cierre) {
                detalles.push(candidato.cierre.motivo);
            }

            if (candidato.id === estado.finalista && estado.oferta) {
                detalles.push(
                    estado.oferta.estado === "Aceptada"
            ...
            return `
                <article class="track-row">
                    ...
                    <span class="track-row__avatar">${hrbpIniciales(candidato.nombre)}</span>

                    <div class="track-row__info">
                        <strong>${candidato.nombre}</strong>
                        <p>${candidato.puesto_actual} · ${candidato.empresa_actual}</p>
                        ${detalles.length > 0 ? `<small>${detalles.join(" · ")}</small>` : ""}
                    </div>
...
function hrbpRenderOferta() {
    ...
    const bloqueado = hrbpEl("offerLocked");
    const formulario = hrbpEl("offerForm");
    ...
    const candidato = EDAT.candidato(estado.finalista);
    ...
    hrbpEl("offerCandidato").value = candidato.nombre;

    if (!hrbpEl("offerPuesto").value) {
        hrbpEl("offerPuesto").value = estado.requisicion?.titulo || "";
    }

    if (!hrbpEl("offerIngreso").value) {
        hrbpEl("offerIngreso").value = EDAT.fecha(EDAT.fechaDemo(30));
    }

    const enviada = Boolean(estado.oferta);

    hrbpEl("offerPill").hidden = !enviada;
    ...
    if (enviada) {
        ...
        hrbpEl("offerPuesto").value = estado.oferta.puesto;
        hrbpEl("offerSueldo").value = estado.oferta.sueldo;
        hrbpEl("offerEsquema").value = estado.oferta.esquema;
        hrbpEl("offerIngreso").value = estado.oferta.ingreso;
        hrbpEl("offerBeneficios").value = estado.oferta.beneficios;

        hrbpEl("offerStatus").innerHTML = [
            {
                titulo: "ESTADO DE LA OFERTA",
                valor: estado.oferta.estado,
                nota: `Enviada el ${EDAT.fecha(estado.oferta.fecha)}`
            },
            {
                titulo: "DISPONIBILIDAD CONFIRMADA",
                valor: estado.oferta.disponibilidad || "En espera",
                nota: estado.oferta.estado === "Aceptada"
                    ? "Confirmada por el candidato"
                    : "El candidato aún no responde"
            },
            {
                titulo: "INCORPORACIÓN",
                valor: estado.incorporacion ? "Confirmada" : "Pendiente",
                nota: estado.incorporacion
...
function hrbpInitOferta() {
    ...
    hrbpEl("offerForm").addEventListener("submit", evento => {

        evento.preventDefault();

        const datos = {
            puesto: hrbpEl("offerPuesto").value.trim(),
            sueldo: hrbpEl("offerSueldo").value.trim(),
            esquema: hrbpEl("offerEsquema").value.trim(),
            ingreso: hrbpEl("offerIngreso").value.trim(),
            beneficios: hrbpEl("offerBeneficios").value.trim()
        };

        if (Object.values(datos).some(valor => valor === "")) {
            hrbpToast("Completa la información de la oferta.");
            return;
        }

        const resultado = EDAT.hrbpEnviarOferta(datos);
...
function hrbpSetVista(vista) {
    ...
    document.querySelectorAll(".view").forEach(seccion => {
        seccion.hidden = seccion.id !== `view-${vista}`;
    });

    document.querySelectorAll(".nav-link").forEach(enlace => {
        enlace.classList.toggle("active", enlace.dataset.view === vista);
    });
...
function hrbpInitNav() {
    ...
    document.querySelectorAll(".nav-link").forEach(enlace => {
        ...
        enlace.addEventListener("click", () => hrbpSetVista(enlace.dataset.view));

    });

    hrbpEl("bellButton").addEventListener("click", hrbpAbrirNotificaciones);
    hrbpEl("footerNoticesButton").addEventListener("click", hrbpAbrirNotificaciones);

    hrbpEl("exitButton").addEventListener("click", EDAT.cerrarSesion);
    hrbpEl("footerExitButton").addEventListener("click", EDAT.cerrarSesion);

    hrbpEl("modalClose").addEventListener("click", hrbpCerrarModal);

    hrbpEl("modalOverlay").addEventListener("click", evento => {

        if (evento.target === hrbpEl("modalOverlay")) {
            hrbpCerrarModal();
        }
...
function hrbpInit() {
    ...
    if (sesion?.email) {
        hrbpEl("hrbpEmail").textContent = sesion.email;
    }
